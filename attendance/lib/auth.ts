...
import { findOfficerInFile, type Officer } from "@/lib/officers";
...
/** Look up an address in the officer allowlist (officers.txt). */
export function findOfficer(address: string): Officer | null {
  return findOfficerInFile(address);
}
...
export async function verifyOfficerLogin(args: {
  ...
  const officer = findOfficer(wallet);
