...
function CheckInFlow() {
  ...
  const validate = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        apiPath(
          `/api/checkin/session/validate?meetingId=${meetingId}&token=${token}`,
        ),
        { cache: "no-store" },
      );
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setKnownName(undefined);
    fetch(apiPath(`/api/members?address=${address}`), { cache: "no-store" })
  ...
  async function checkIn() {
    ...
    try {
      ...
      const chRes = await fetch(apiPath("/api/checkin/challenge"), {
        method: "POST",
      ...
      const res = await fetch(apiPath("/api/checkin"), {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          meetingId,
